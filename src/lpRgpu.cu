...
lpRgpu::lpRgpu(int* N,float* fZ,int* st,int* stadj,int* idthetatx,int* idrhotx,int* idthetatauq,int* idrhotauq,float* dthetatx,
			   float* drhotx,float* dthetatauq,float* drhotauq,float* emul,float* cosmul,float* J,int* reorids,int* reoridsadj)
{
    ...
    if (ni>=64) { MBS31=4;MBS32=4;MBS33=64;}
    else if (ni>=16) { MBS31=8;MBS32=8;MBS33=16;}
    else if (ni>=4) { MBS31=16;MBS32=16;MBS33=4;}
    else { MBS31=32;MBS32=32;MBS33=1;}    
}
lpRgpu::~lpRgpu()
{
	cudaFree(dftx);
...
void lpRgpu::fwd(float* out,float* in)
{
...
//mul erho,J
	...
	pmul <<<dimGrid0,dimBlock>>>(dftx,dJ,Nt,Nx,ni);
	pmul <<<dimGrid0,dimBlock>>>(dftx,demul,Nt,Nx,ni);	
//convtx
	GS31=(int)ceil(Ntheta/(float)MBS31);GS32=(int)ceil(Nrho/(float)MBS32);GS33=(int)ceil(ni/(float)MBS33);dim3 dimGrid1(GS31,GS32,GS33);	
	convtx_ker<<<dimGrid1,dimBlock>>>(dfl,dftx,dst,ddthetatx,ddrhotx,dreorids,Nt,Nx,Ntheta,Nrho,ni);

//fftlp

	cufftExecR2C(plan_forward, (cufftReal*)&dfl[(Ntheta+2*add)*add+add],(cufftComplex*)dflc);
	...
	mul<<<dimGrid2, dimBlock>>>(1/(float)(Ntheta*Nrho),dflc,dfZ,Ntheta_R2C,Nrho,ni);
	cufftExecC2R(plan_inverse,(cufftComplex*)dflc,(cufftReal*)&dfl[(Ntheta+2*add)*add+add]);	
	

	//convtauq

	GS31=(int)ceil(Nq/(float)MBS31);GS32=(int)ceil(Ntau/(float)MBS32);GS33=(int)ceil(ni/(float)MBS33);dim3 dimGrid3(GS31,GS32,GS33);	
	convtauq_ker<<<dimGrid3,dimBlock>>>(dftauq,dfl,didthetatauq,didrhotauq,ddthetatauq,ddrhotauq,dreoridsadj,Nq,Ntau,Ntheta,Nrho,ni);		
//mul cos
	pmul <<<dimGrid3,dimBlock>>>(dftauq,dcosmul,Nq,Ntau,ni);


	err=cudaMemcpy(out,dftauq,Nq*Ntau*ni*sizeof(float),cudaMemcpyDefault);if (err!=0) callErr(cudaGetErrorString(err));	
}
void lpRgpu::adj(float* out,float* in)
{
	...
	pmul <<<dimGrid0,dimBlock>>>(dftauq,dcosmul,Nq,Ntau,ni);
	//conv
	cudaMemset(dfl,0,(Ntheta+2*add)*(Nrho+2*add)*ni*sizeof(float));	
	GS31=(int)ceil(Ntheta/(float)MBS31);GS32=(int)ceil(Nrho/(float)MBS32);GS33=(int)ceil(ni/(float)MBS33);dim3 dimGrid1(GS31,GS32,GS33);	
	convtx_ker<<<dimGrid1,dimBlock>>>(dfl,dftauq,dstadj,ddthetatauq,ddrhotauq,dreoridsadj,Nq,Ntau,Ntheta,Nrho,ni);
	//fftlp
	cufftExecR2C(plan_forward, (cufftReal*)dfl,(cufftComplex*)dflc);	
	GS31=(int)ceil(Ntheta_R2C/(float)MBS31);GS32=(int)ceil(Nrho/(float)MBS32);GS33=(int)ceil(ni/(float)MBS33);dim3 dimGrid2(GS31,GS32,GS33);
	muladj<<<dimGrid2, dimBlock>>>(1/(float)(Ntheta*Nrho),dflc,dfZ,Ntheta_R2C,Nrho,ni);
	...
	convtauq_ker<<<dimGrid3,dimBlock>>>(dftx,dfl,didthetatx,didrhotx,ddthetatx,ddrhotx,dreorids,Nt,Nx,Ntheta,Nrho,ni);
	//mul erho,J
	pmul <<<dimGrid3,dimBlock>>>(dftx,dJ,Nt,Nx,ni);
	pmul <<<dimGrid3,dimBlock>>>(dftx,demul,Nt,Nx,ni);	
...
void lpRgpu::fftlp(float* out,float* in)
{
	...
	mul<<<dimGrid, dimBlock>>>(1/(float)(Ntheta*Nrho),dflc,dfZ,Ntheta_R2C,Nrho,ni);
	//inverse FFT
	cufftExecC2R(plan_inverse,(cufftComplex*)dflc,(cufftReal*)dfl);
...
void lpRgpu::fftlpadj(float* out,float* in)
{
	...
	dim3 dimBlock(MBS31,MBS32,MBS33);	
	int GS31=(int)ceil(Ntheta_R2C/(float)MBS31);int GS32=(int)ceil(Nrho/(float)MBS32);int GS33=(int)ceil(ni/(float)MBS33);dim3 dimGrid(GS31,GS32,GS33);
	muladj<<<dimGrid, dimBlock>>>(1/(float)(Ntheta*Nrho),dflc,dfZ,Ntheta_R2C,Nrho,ni);
	//inverse FFT
	cufftExecC2R(plan_inverse,(cufftComplex*)dflc,(cufftReal*)dfl);
...
void lpRgpu::convtx(float* out,float* in)
{
	...
	dim3 dimBlock(MBS31,MBS32,MBS33);	
	int GS31=(int)ceil(Ntheta/(float)MBS31);int GS32=(int)ceil(Nrho/(float)MBS32);int GS33=(int)ceil(ni/(float)MBS33);dim3 dimGrid(GS31,GS32,GS33);	
	convtx_ker<<<dimGrid,dimBlock>>>(dfl,dftx,dst,ddthetatx,ddrhotx,dreorids,Nt,Nx,Ntheta,Nrho,ni);
	err=cudaMemcpy(out,dfl,Ntheta*Nrho*ni*sizeof(float),cudaMemcpyDefault);if (err!=0) callErr(cudaGetErrorString(err));	
}
void lpRgpu::convtauq(float* out,float* in)
{
	...
	convtauq_ker<<<dimGrid,dimBlock>>>(dftauq,dfl,didthetatauq,didrhotauq,ddthetatauq,ddrhotauq,dreoridsadj,Nq,Ntau,Ntheta,Nrho,ni);
	err=cudaMemcpy(out,dftauq,Nq*Ntau*ni*sizeof(float),cudaMemcpyDefault);if (err!=0) callErr(cudaGetErrorString(err));	
}
...
void lpRgpu::convtauqadj(float* out,float* in)
{
	...
	MBS31=8;MBS32=8;MBS33=16;
	dim3 dimBlock(MBS31,MBS32,MBS33);	
	int GS31=(int)ceil(Ntheta/(float)MBS31);int GS32=(int)ceil(Nrho/(float)MBS32);int GS33=(int)ceil(ni/(float)MBS33);dim3 dimGrid(GS31,GS32,GS33);	
	convtx_ker<<<dimGrid,dimBlock>>>(dfl,dftauq,dstadj,ddthetatauq,ddrhotauq,dreorids,Nq,Ntau,Ntheta,Nrho,ni);
	err=cudaMemcpy(out,dfl,Ntheta*Nrho*ni*sizeof(float),cudaMemcpyDefault);if (err!=0) callErr(cudaGetErrorString(err));	
}
...
void lpRgpu::convtxadj(float* out,float* in)
{
	...
	convtauq_ker<<<dimGrid,dimBlock>>>(dftx,dfl,didthetatx,didrhotx,ddthetatx,ddrhotx,dreoridsadj,Nt,Nx,Ntheta,Nrho,ni);
	err=cudaMemcpy(out,dftx,Nt*Nx*ni*sizeof(float),cudaMemcpyDefault);if (err!=0) callErr(cudaGetErrorString(err));	
}
